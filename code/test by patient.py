...
def test_by_patient(patient_path_list):
    best_models_dict = {"A2C": "3 CLASS Final/single_view_models/A2C/A2C_best_model_1_e50_lr=0.001.pth",
                        "A3C": "3 CLASS Final/single_view_models/A3C/A3C_best_model_1_e50_lr=0.001.pth",
                        "A4C": "3 CLASS Final/single_view_models/A4C/A4C_best_model_2_e50_lr=0.0001.pth",
                        }
    ...
    for patient_path in patient_path_list:
        ...
        if len(result) != 1 and len(number) != 0:
            count_list = []
        ...
        if final_result:
            ...
            if final_result != truth:
                wrong_list.append([[patient_name], ["predict:", final_result, "Truth:", truth]])

            print("{} 预测时间为：{:.1f} ms".format(patient_name, 1000 * patient_time))
            print("****************************************")

    #  step 6:打印输出结果
    total_sample = np.sum(patient_confusion.matrix)
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    print(len(all_patient_path_list))
